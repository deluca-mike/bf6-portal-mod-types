...
- **bf6-portal-mod-types** `4.2.0` corresponds to the official **1.4.2.0** `mod` namespace.
- **Patch** releases (e.g. `4.2.1`, `4.2.2`) = same official API, with additional JSDoc or usability improvements only.
- When the Portal team releases a new SDK version (e.g. `1.4.3.0`), this package would move to a new **minor** (e.g.
  `4.3.0`) and the root types would be updated from the new official `index.d.ts`.
